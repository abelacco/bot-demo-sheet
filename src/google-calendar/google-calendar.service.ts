...
export class GoogleCalendarService {
  ...
  async findAvailableSlots(
    calendarId: string = process.env.GOOGLE_EMAIL_ACCOUNT,
    dateString: string,
    ...
    try {
      ...
      while (currentTime < timeMax) {
        ...
        console.log(
          `Checking slot: ${currentTime.format(format)} to ${endTime.format(
            format,
          )}`,
        );
        ...
        if (date === todayDate && now.isSameOrAfter(currentTime, 'minute')) {
          console.log(
            `Slot ${currentTime.format(format)} to ${endTime.format(
              format,
            )} has passed.`,
          );
        ...
        const isSlotAvailable = events.every((event) => {
          ...
          return (
            endTime.isSameOrBefore(eventStart) ||
            currentTime.isSameOrAfter(eventEnd)
          );
        });
  ...
  async createEventWithGoogleMeetAndNotifyAttendees(
    ...
    try {
      ...
      console.error(
        'Error al crear evento en Google Calendar con Google Meet y notificar a los asistentes:',
        error,
      );
      throw new Error(
        'Failed to create event in Google Calendar with Google Meet and notify attendees',
      );
    }
  }
  ...
  async updateEventStatusToCancelled(
    calendarId: string = process.env.GOOGLE_EMAIL_ACCOUNT,
    eventId: string,
  ): Promise<string> {
    try {
