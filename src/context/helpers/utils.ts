...
export class Utilities {
  ...
  static convertSchedule(schedule) {
    ...
    const convertedDay = moment(schedule.day, 'YYYY-MM-DD').format(
      'dddd D [de] MMMM',
    );
  ...
  static parseAvailableSpots(schedule: { day: string; hours: string[] }) {
    if (!schedule.day)
      return 'El cliente no ha especificado un día ni hora de manera clara, preguntar para que día y hora desea tener la asesoría'; // Retorna un mensaje por defecto si no hay horarios
  ...
  static generateOneSectionTemplate(
    menuTitle: string,
    spots: any,
  ...
  static isWithinBusinessHours(dateTimeString, timeZone = 'America/Lima') {
    ...
    const isWithinHours =
      dateTime.isSameOrAfter(startTime) && dateTime.isBefore(endTime);
  ...
  static validateBusinessInfo(info) {
    ...
    if (missingFields.length > 0) {
      return (
        'Por favor, faltan los siguientes datos: ' + missingFields.join(', ')
      );
    } else {
      return 'OK';
    }
   ...
   */
  static parseSpanishDateTimeToMoment(
    dateStr: string,
    year: number = 2024,
  ...
  static parseIdFromInteractiveButtons(id) {
    // Divide el id por el carácter ':'
    const parts = id.split(':');

    // Verifica que se hayan obtenido exactamente dos partes
    if (parts.length === 2) {
      return {
        prop: parts[0], // La primera parte, antes de los dos puntos o propiedad en la base de datos
        value: parts[1], // La segunda parte, después de los dos puntos el valor de la propiedad que se guardará en la base de datos
      };
    } else {
      // Lanza un error si el formato no es el esperado
      throw new Error(
        'Button interactive ID format is incorrect. Expected format "type:value".',
      );
    }
  }
}
